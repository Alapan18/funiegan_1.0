...
class GeneratorFunieGAN(nn.Module):
    def __init__(self, in_channels=3, out_channels=3):
        ...
        # Attention layers using MultiHeadSelfAttention
        self.attention1 = MultiHeadSelfAttention(256)  # After down3
        self.attention2 = MultiHeadSelfAttention(256)  # After up3
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        proj_query = self.query_conv(x).view(batch_size, self.num_heads, head_dim, width * height)
        proj_key = self.key_conv(x).view(batch_size, self.num_heads, head_dim, width * height)
        proj_value = self.value_conv(x).view(batch_size, self.num_heads, head_dim, width * height)

        energy = torch.einsum("bncf, bndf -> bnc", proj_query, proj_key)
        attention = self.softmax(energy / head_dim**0.5)

        out = torch.einsum("bnc, bndf -> bncf", attention, proj_value).view(batch_size, C, width, height)

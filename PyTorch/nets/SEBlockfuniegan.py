...
class GeneratorFunieGAN(nn.Module):
    def __init__(self, in_channels=3, out_channels=3):
        ...
        # Attention layers using SEBlock
        self.attention1 = SEBlock(256)  # After down3
        self.attention2 = SEBlock(256)  # After up3
...
class SEBlock(nn.Module):
    def __init__(self, in_dim, reduction=16):
        ...
        self.fc1 = nn.Linear(in_dim, in_dim // reduction)
        self.fc2 = nn.Linear(in_dim // reduction, in_dim)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        batch_size, C, _, _ = x.size()
        se = F.adaptive_avg_pool2d(x, 1).view(batch_size, C)
        se = F.relu(self.fc1(se))
        se = self.sigmoid(self.fc2(se)).view(batch_size, C, 1, 1)
        return x * se
